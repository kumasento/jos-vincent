...
alloc_block(void)
{
	...
	uint32_t blockno;
	// skip block 0
	for (blockno = 1; blockno < BLKBITSIZE; blockno ++) {
		if (bitmap[blockno/32] & (1<<(blockno%32))) {// this block is free
			bitmap[blockno/32] &= ~(1<<(blockno%32)); // now this block is allocated
			flush_block(bitmap); 
			return blockno;
		}
	}
